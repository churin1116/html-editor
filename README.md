...
- **Hybrid HTML / Markdown** — `.html` files are edited directly and stay viewable via `file://` (self-contained CSS embedded in each saved file). `.md` files are converted to HTML on read and back to Markdown on save, so existing `.md` notes keep their format and remain `git diff`-friendly.
...
Saved as a self-contained document so they render directly in any browser:
...
<head>
  <meta charset="utf-8">
  <title>...</title>
  <style>/* embedded prose CSS */</style>
</head>
<body>
...
Only the inner `<article id="content">` is editable; the surrounding template (and embedded CSS) is regenerated on every save.
