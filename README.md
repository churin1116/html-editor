...
- **Selection toolbar** — In both HTML editing modes, selecting text pops a floating toolbar (bold / italic / strike / inline code / H2 / H3 / lists / quote / link / clear formatting), so common formatting never requires the sidebar menu. Right-clicking in the document opens the same toolbar in place of the browser's context menu — above the selection if you right-click it, otherwise at the pointer with the caret moved there — and any click elsewhere, keystroke or scroll dismisses it. Opened by right-click, it also offers "目次" (insert a TOC there). Shift+right-click keeps the native menu (copy/paste, spelling); right-clicking a `<details>` summary still opens its initial-state menu.
...
- **Table of contents** — "目次" in the action menu (or the right-click toolbar) inserts a note-style TOC block listing the document's h2/h3 as in-page links (h3 nested under its h2), beside the current block — an empty paragraph is replaced, otherwise it goes after the block the caret is in. It updates live as headings change, and clicking an entry scrolls the editor to that heading. Like link cards, the list is written into the saved file as plain markup — so the TOC works with no script when the file is opened on its own — and is regenerated from the headings on every save, so editing it by hand doesn't stick. While a document contains a TOC, every h2/h3 gets a short stable `id` (`h-xxxxxx`) for the links to target; these are saved with the headings. Works in both HTML views: in the rendered view of hand-written files it is inserted by right-click (`src/lib/toc-dom.ts`, same markup) and kept current by a mutation observer while you edit. Not available for `.md`. A right-docked TOC *sidebar* for any Chameleon page, this editor included, comes from the Chameleon Chrome extension instead.
